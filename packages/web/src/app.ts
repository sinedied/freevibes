...
export class App extends LitElement {
	...
	async connectedCallback() {
		...
		this.githubLoggedIn = Boolean(githubToken);
	...
	private async handleGithubLogin(loginEvent: Event) {
		...
		try {
			...
			this.loginError = 'Login failed: ' + errorMessage;
	...
	private async handleAddWidget(event: CustomEvent) {
		...
		const minOrder =
			columnWidgets.length > 0 ? Math.min(...columnWidgets.map((w) => w.position.order)) : WIDGET_ORDER_SPACING;
	...
	private async handleEditWidget(event: CustomEvent) {
		...
		const updatedWidgets = this.data.widgets.map((w) => {
			...
			if (nextTabId !== w.tabId) {
				const { column } = w.position;
				const columnWidgets = this.data!.widgets.filter(
					(widget) => widget.tabId === nextTabId && widget.position.column === column,
				);
				const minOrder =
	...
	private async handleAddTab(event: CustomEvent) {
		...
		const maxOrder = this.data.tabs.length > 0 ? Math.max(...this.data.tabs.map((t) => t.order)) : 0;
	...
	private async handleEditTabSave(event: CustomEvent) {
		...
		const updatedTabs = this.data.tabs.map((t) => (t.id === config.id ? { ...t, name: config.name } : t));
			...
			return html`
				...
				<form class="login-form" @submit=${this.handleGithubLogin}>
					...
					<div class="login-help">
						...
						Go to
						<a
							href="https://github.com/settings/tokens/new?scopes=gist&description=FreeVibes%20Dashboard"
							target="_blank"
							rel="noopener"
		...
		return html`
			<div class="header">
				...
				${this.data.tabs.length > 0
					...
					: ''}
				<nav class="nav">
					<button class="menu-btn" @click=${this.toggleMenu} title="Menu">${unsafeSVG(menuIcon)}</button>
					${this.showMenu
						? html`
