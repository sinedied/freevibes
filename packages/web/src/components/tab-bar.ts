...
export class TabBar extends LitElement {
	...
	private handleDrop(event: DragEvent) {
		...
		for (const [index, tab] of reorderedTabs.entries()) {
			tab.order = (index + 1) * 1000;
		}

		this.dispatchEvent(
			new CustomEvent('tabs-reordered', {
	...
	render() {
		const sortedTabs = [...this.tabs].sort((a, b) => a.order - b.order);
		const draggedIndex = this._draggedTabId ? sortedTabs.findIndex((t) => t.id === this._draggedTabId) : -1;

		return html`
			<div class="tab-bar" @drop=${this.handleDrop} @dragleave=${this.handleDragLeave}>
				${sortedTabs.map((tab, index) => {
					const isActive = tab.id === this.currentTabId;
					const isDragging = tab.id === this._draggedTabId;
					const showIndicatorBefore =
						this._dropTargetIndex === index && draggedIndex !== index && draggedIndex !== index - 1;
				...
				<div
					class="drop-zone-end"
					@dragover=${(e: DragEvent) => {
						this.handleDragOver(e, sortedTabs.length);
					}}
				></div>
			</div>
		`;
