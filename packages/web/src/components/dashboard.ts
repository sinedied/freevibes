...
export class Dashboard extends LitElement {
	...
	static styles = css`
		...
		.resize-handle:hover {
			...
			background: linear-gradient(
				...
				transparent 0%,
				transparent 30%,
				var(--fv-accent-primary) 30%,
				var(--fv-accent-primary) 40%,
				transparent 40%,
				transparent 60%,
				var(--fv-accent-primary) 60%,
				var(--fv-accent-primary) 70%,
				transparent 70%
			);
		}
	...
	private handleWidgetUpdate(event: CustomEvent) {
		...
		const updatedData = {
			...this.data,
			widgets: this.data.widgets.map((w) => (w.id === updatedWidget.id ? updatedWidget : w)),
		};
	...
	private async initializeDragHandlers() {
		...
		if (widgets)
			for (const widgetEl of widgets) {
				...
				if (rssComponent) {
					headerEl = rssComponent.shadowRoot?.querySelector('.header') ?? undefined;
				} else if (notesComponent) {
					headerEl = notesComponent.shadowRoot?.querySelector('.header') ?? undefined;
				}

				if (headerEl) {
					headerEl.setAttribute('draggable', 'true');
					headerEl.addEventListener('dragstart', (e) => {
						const widgetId = (widgetEl as HTMLElement).dataset.id;
						if (widgetId) {
	...
	private handleDragOver(e: DragEvent, columnIndex: number, position?: number) {
		...
		if (position === undefined) {
			...
			for (const [i, widget] of widgets.entries()) {
				const widgetEl = this.shadowRoot?.querySelector(`[data-id="${widget.id}"]`);
				if (widgetEl) {
		...
		if (this._dragPreview?.column !== newPreview.column || this._dragPreview.position !== newPreview.position) {
	...
	private handleDrop(e: DragEvent) {
		...
		for (const [index, widget] of newColumnWidgets.entries()) {
	...
	private renderDragPlaceholder(widget: Widget) {
		return html` <div class="drag-placeholder" style="--placeholder-height: ${widget.height || 6}"></div> `;
	}
	...
	private readonly handleResizeStart = (e: MouseEvent, widgetId: string) => {
		e.preventDefault();
	...
	private readonly handleResize = (e: MouseEvent) => {
	...
	private readonly handleResizeEnd = () => {
		...
		if (widget) {
			...
			const updatedData = {
				...this.data,
				widgets: this.data.widgets.map((w) => (w.id === this._resizingId ? { ...w, height: widget.height } : w)),
			};
			...
			>
				...
				<div
					...
					@mousedown=${(e: MouseEvent) => {
						this.handleResizeStart(e, widget.id);
					}}
				></div>
			</div>
		`;
	...
	protected updated() {
		...
		void this.initializeDragHandlers();

		// Organize widgets by column for easier access
		const { columns } = this.data.settings;
	...
	render() {
		...
		if (tabWidgets.length === 0) {
			return html`
				<div class="dashboard">
					<div class="empty-state">No widgets configured. Add some widgets to get started!</div>
				</div>
			`;
		}
		...
		return html`
			<div class="dashboard">
				${columnArrays.map(
					(columnWidgets, columnIndex) => html`
						<div
							class="column"
							data-column=${columnIndex + 1}
							@dragover=${(e: DragEvent) => {
								this.handleDragOver(e, columnIndex);
							}}
