...
export class EditWidgetDialog extends LitElement {
	...
	static styles = css`
		...
		.color-option.selected {
			...
			box-shadow:
				0 0 0 2px var(--fv-bg-secondary),
				0 0 0 4px var(--fv-accent-primary);
		}
		...
		return html`
					...
					? html`
							...
							<div class="form-group">
								...
								<div class="color-picker">
									${(['yellow', 'green', 'blue', 'red'] as NoteColor[]).map(
										(c) => html`
											<div
						...
						`
					: ''}
				${this.isEditMode() && this.tabs.length > 1
					? html`
			...
			<div class="footer">
				<div class="footer-left">
					${this.isEditMode() ? '' : html` <button class="btn back-btn" @click=${this.handleBack}>Back</button> `}
					${this.isEditMode()
						? html`
								<button class="btn btn-danger" @click=${this.handleDeleteClick}>${unsafeSVG(trashIcon)} Delete</button>
							`
						: ''}
				</div>
					...
					<div class="header">
						<h2 class="title">
							${this.step === 'select'
								? 'Add Widget'
								: this.isEditMode()
									? `Edit ${this.selectedType === 'rss' ? 'RSS Feed' : 'Note'}`
									: `Configure ${this.selectedType === 'rss' ? 'RSS Feed' : 'Note'}`}
						</h2>
