...
export class Settings extends LitElement {
	...
	private handleColumnsChange(event: Event) {
		...
		this.columns = Number.parseInt(target.value, 10);
	}
	...
	private handleFontSizeChange(event: Event) {
		const target = event.target as HTMLInputElement;
		this.fontSize = Number.parseInt(target.value, 10);
	}
	...
	private async save() {
		const updatedData = {
			...
			settings: {
				...
				darkMode:
					this.darkModeType === 'on' ||
					(this.darkModeType === 'system' && globalThis.matchMedia('(prefers-color-scheme: dark)').matches),
				...
				<div class="modal">
					...
					<div class="content">
						<div class="setting-group">
							...
							<div class="setting-description">Adjust the number of columns in the dashboard grid</div>
							<input
								type="range"
								class="range-input"
								min="1"
						...
						<div class="setting-group">
							...
							<div class="setting-description">Choose the main accent color for buttons and links</div>
							<div class="color-input-group">
								<input type="color" class="color-input" .value=${this.mainColor} @input=${this.handleMainColorChange} />
								<input
									type="text"
									class="color-hex"
						...
						<div class="setting-group">
							...
							<div class="setting-description">Choose the background color (only applies in light mode)</div>
							<div class="color-input-group">
								<input
									type="color"
									class="color-input"
						...
						<div class="setting-group">
							...
							<div class="setting-description">Adjust the base font size for better readability</div>
							<input
								type="range"
								class="range-input"
								min="12"
						...
						<div class="setting-group">
							...
							<div class="setting-description">Export or view your dashboard configuration</div>
							<div class="actions-left">
								<button class="btn btn-secondary" @click=${this.downloadConfig}>
									${unsafeSVG(downloadIcon)} Download Config
								</button>
								${dataService.isGistEnabled()
									? html`
						...
						<div class="info">
							...
							<p class="info-text">
								Your dashboard data is automatically saved to your browser's local storage. To sync across devices or
								backup your data, you can export your configuration and add it to a GitHub repository.
							</p>
						</div>
					</div>
					...
					<div class="actions">
						<div class="actions-right">
							...
							<button class="btn btn-primary" @click=${this.save}>Save Changes</button>
						</div>
					</div>
				</div>
			</div>
